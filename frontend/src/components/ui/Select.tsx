...
export function Select({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <select
        id={selectId}
        className={`input appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2216%22%20height%3D%2216%22%20fill%3D%22%236b7280%22%20viewBox%3D%220%200%2016%2016%22%3E%3Cpath%20d%3D%22M8%2011%204%207h8l-4%204z%22%2F%3E%3C%2Fsvg%3E')] bg-[position:right_12px_center] bg-no-repeat pr-9 ${error ? 'input-error' : ''} ${className}`}
        aria-invalid={!!error}
        {...rest}
      >
        {options.map((o) => (
