...
export const NAV_ITEMS: NavItem[] = [
  ...
  { to: '/new', label: '新建任务', icon: 'plus' },
  { to: '/assets', label: '资产库', icon: 'folder', disabled: true },
  { to: '/publish', label: '发布物料', icon: 'send', disabled: true },
  { to: '/settings', label: '设置', icon: 'gear', disabled: true },
]
        ...
        const inner = (
          <>
            ...
            {!collapsed && item.disabled && (
              <span className="ml-auto rounded-sm bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-400">
                敬请期待
              </span>
            )}
          </>
        ...
        if (item.disabled) {
          return (
            <span
              key={item.to}
              title={item.label}
              className="flex cursor-not-allowed items-center rounded-md px-3 py-2.5 text-sm font-medium text-gray-300"
            >
              {inner}
            </span>
          )
